...
{
    $stat = 'Fail'
    Write-Host "2.3.6.2 (L1) Ensure 'Domain member: Digitally encrypt secure channel data (when possible)"
...
function _18.3.6.WDigestUseLogonCredential{
    $stat = 'Fail'
    Write-Host "18.3.6 (L1) Ensure 'WDigest Authentication' is set to 'Disabled'"
...
##18.5.4 DNS Client
function _18.5.4.1.NetBIOSNodeType{
    $stat = 'Fail'
    Write-Host "18.5.4.1 (L1) Set 'NetBIOS node type' to 'P-node' (Ensure NetBT Parameter 'NodeType' is set to '0x2 (2)')"
    $reg_p = "HKLM:\SYSTEM\CurrentControlSet\Services\NetBT\Parameters"
    $reg_n = "NodeType"
    try{ $val = Get-ItemPropertyValue $reg_p -Name $reg_n -ErrorAction SilentlyContinue}catch{$val = ""}
    Write-Host "Command:  Get-ItemPropertyValue $reg_p -Name $reg_n"
...
function _18.5.4.2.EnableMulticast{
    $stat = 'Fail'
    Write-Host "18.5.4.2 (L1) Ensure 'Turn off multicast name resolution' is set to 'Enabled'"
...
function _18.5.21.1.fMinimizeConnections{
    $stat = 'Fail'
    Write-Host "18.5.21.1 (L1) Ensure 'Minimize the number of simultaneous connections to the Internet or a Windows Domain' is set to 'Enabled'"
...
function _18.9.86.1.DisableAutomaticRestartSignOn{
    $stat = 'Fail'
    Write-Host "18.9.86.1 (L1) Ensure 'Sign-in last interactive user automatically after a system-initiated restart' is set to 'Disabled'"
    $reg_p = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System"
...
function Level_1_Member_Server{
    ...
    _18.5.4.1.NetBIOSNodeType		 
    _18.5.4.2.EnableMulticast
...
function Level_1_Domain_Controller_Level_1_Member_Server{
    ...
    _18.3.6.WDigestUseLogonCredential	
